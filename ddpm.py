...
class Diffusion(object):
    ...
    def generate(self):
        ...
        self.net = GaussianDiffusion(
            UNet(3, self.channel), self.input_shape, 3, betas=betas
        )
    ...
    def generate_5x5_image(self, save_path):
        with torch.no_grad():
            ...
            for i, j in itertools.product(
                range(size_figure_grid), range(size_figure_grid)
            ):
            ...
            for k in range(5 * 5):
                ...
                ax[i, j].imshow(
                    np.uint8(
                        postprocess_output(
                            test_images[k].cpu().data.numpy().transpose(1, 2, 0)
                        )
                    )
                )
    ...
    def generate_1x1_image(self, save_path):
        with torch.no_grad():
            ...
            test_images = postprocess_output(
                test_images[0].cpu().data.numpy().transpose(1, 2, 0)
            )
